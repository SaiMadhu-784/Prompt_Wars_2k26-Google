...
API_KEY = None
if fernet_key and encrypted_api_key:
    try:
        cipher = Fernet(fernet_key.encode())
        API_KEY = cipher.decrypt(encrypted_api_key.encode()).decode()
    except Exception as e:
        st.error(f"🚨 Failed to decrypt API Key: {e}")

if not API_KEY:
    st.error("⚠️ **Critical Security Halt**: Missing or invalid `FERNET_KEY` and `ENCRYPTED_API_KEY` in your `.env` file. Please run `python encrypt_tool.py` to generate them securely.")
    st.stop()
